...
public record RoutingFrame(
    ...
    public static byte[] toBytes(byte[] frameData) {
        return new RoutingFrame(frameData, null, null, null).toBytes();
    }

    @Override
    public byte[] toBytes() {
        return IFrame.RoutingFrame.newBuilder()
    ...
    public static class Coder implements FrameCoder {
        ...
        public Frame decode(AlpsMetadata metadata, AlpsData data, AlpsProtocol.AlpsPacket rawPacket) throws Exception {
            ...
            return new RoutingFrame(frame.getData().toByteArray(), metadata, data, rawPacket);
        }
